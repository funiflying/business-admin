...
export function fetch_company_list({page=1,size=10,status='',name=''}) {
    return request(constant.SERVICE_COMPANY_LIST+`?pageNo=${page}&pageNum=${size}&name=${name}&status=${status}`)
}
...
export function fetch_company_app_list({page=1,size=20,name='',status=''}) {
    return request(constant.SERVICE_COMPANY_APP_LIST+`?pageNo=${page}&pageNum=${size}&name=${name}&status=${status}`)
}
export function audit_company_app({id,status,reply,deviceType,deviceCount,eid}) {
    return request(constant.SERVICE_COMPANY_APP_AUDIT+`?id=${id}&status=${status}&reply=${reply}&deviceType=${deviceType}&deviceCount=${deviceCount}&eid=${eid}`)
}
